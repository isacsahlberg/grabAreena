...
def parser_():
    ...
    parser.add_argument(
        "--pattern",
        "-p",
        help='set specific search pattern string (default: "Bach", "Mozart" and "Schumann")',
    )
    parser.add_argument(
        "--tomorrow", "-tmrw", action="store_true", help='force date to be "tomorrow"'
    )
    parser.add_argument(
        "--nocolor",
        "-nc",
    ...
    parser.add_argument(
        ...
        "-gu",
        action="store_true",
        help="print url for easy copy-pasting and checking",
    )
    parser.add_argument(
        "--programs",
        "-gp",
...
def getContents(html_str, program_times):
    ...
    program_contents = [
        program_times[i][0] + " " + list_contents[i]
        for i, _ in enumerate(list_contents)
    ]
    # Let's also calculate how many pieces each program has
    program_lengths = [len(re.split("\d+:\d\d ", p)[1:]) for p in program_contents]
...
def printPrograms(programs, program_lengths, program_times, endtime, today):
    print(f"\n\n\n----> Here are the programs for the {day_str(today)}:\n")
    # For aligning the output, let's find the longest program name
    max_len = max([len(p) for p in programs])
    for p, l, (t1, t2) in zip(programs, program_lengths, program_times):
        print(f"{t1.rjust(6)} -{t2.rjust(6)}  --  {p.ljust(max_len)} (pieces: {l})")
...
def printMatches(matches, linebreaks, pattern, today):
    ...
    if N_matches > 0:
        print(
            "\n" * 4 + f"----> There are {N_matches} entries matching {pattern_str} ",
            end="",
        )
